...
import { SlSocialDropbox, SlAnchor, SlCloudDownload, SlWrench, SlCloudUpload} from "react-icons/sl";
...
In the previous step, we overrode the example plugin to use our local plugin. Now let's look at how to host your plugin locally so it shows up by deafult in the 1FE app.
...
:::tip
The bathtub widget build will also generate assets in the `dist` folder.
:::
...
4. Create a new directory within `integration/widgets/@1fe/bathtub` of your forked cdn repo. The directory name should be the version from the package.json of the bathtub repository (e.g `1.0.50`).

5. Upload the contents of the `1fe-bathtub/dist/` directory to the bathtub directory. Following is an image which showcases what the directory structure will look like after you have moved the code over.
...
5. Confirm that the updated Live Configurations are working by visiting the following link:
```
...
You need to update the CSP settings in the 1FE app(`src/csp-configs.ts`) to allow them(`http://localhost:3000/`).
:::
