...
Now that we have set up our local 1fe instance and widget starter kit, it's time to deploy our setup to showcase its benefits. This will enable us to share our work with others and demonstrate the power of 1fe. We will do the following in this guide.

1.  **Deploy our assets to a public CDN (JSDelivr).**
2.  **Deploy our 1fe instance to a hosting service.**

### Set up the required GitHub repositories

You need two GitHub repositories:

**CDN Assets Repository**

You need a repository for the `mock-cdn-assets` that will hold your assets. We will be using JSDelivr to serve these assets. JSDelivr works off of GitHub repositories and serves files based on git tags. You've already created a repository for your forked CDN assets on github.com and pushed your changes to that repository as part of the guide to get your setup locally. If you haven't done that, do that now so our mock-cdn-assets are available on github.com for JSDelivr to pick up from.
...
You have already learned how to host assets locally in the previous guide. Let's deploy to them a CDN so your hosted 1fe instance can access them. For this process, you will need to

### Configure the 1fe instance first

For this section, let's assume that you are going to use a git tag for your CDN assets, like `v1.0.2`. You can pick any tag you like, but make sure to use the same tag in the next steps. We will use `<tag>` to refer to this tag in the rest of this guide. Based on this information (and with some more steps that you will take later), you need to make the following changes to your 1fe instance files.
...
3.  Update the CSP policies as defined in `src/csp-configs.ts` to allow jsdelivr domains(`https://cdn.jsdelivr.net`)
...
When working on getting your setup to work locally, you already uploaded bundles for `widget-starter-kit` and `playground` to the CDN. You need to upload the shell bundle.
...
Let's now add all the changes you have made to your CDN assets repository and create a git tag for them. This is crucial for JSDelivr to cache the assets and serve them correctly.
...
## Deploy your 1fe instance to a hosting service

Now that you have our CDN setup, let's deploy your 1fe instance to a hosted service such that it's pointing to your CDN.
