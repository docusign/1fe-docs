...
• **Hosting Service:** Where your 1fe application will run ([Azure Web App explained here](/infrastructure/hosting-setup/))

• **Deployment Target:** CDN or cloud storage for hosting widget bundles ([CDN Setup explained here](/infrastructure/cdn-setup/))

• **Configuration Management:** Key-value stores for live configuration settings ([Configuration Management explained here](/infrastructure/configuration-setup/))

• **Private NPM Registry:** Internal registry for publishing and consuming widget packages ([NPM Registry Setup explained here](/infrastructure/npm-registry-setup/))
...
• **CDN Deployment Secret** (`AKAMAI_NS_SSH_PRIVATE_KEY`) - SSH private key for Akamai CDN deployment. This allows the pipeline to upload your built widget files to the content delivery network.

• **Configuration Service Secret** (`AZURE_APP_CONFIG_CONNECTION_STRING`) - Azure App Configuration connection string. This enables the pipeline to update configuration metadata when your widget is deployed.

• **Package Registry Secrets** - Authentication tokens for the internal NPM registry:
