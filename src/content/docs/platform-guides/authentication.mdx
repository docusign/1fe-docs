...
- `isAuthedCallback` is invoked when the shell is loading a plugin that requires authentication. 
- `unauthedCallback` is invoked when `isAuthedCallback` returns `false`.
...
Let's assume a cookie-less token-based authentication model. Here is an example snippet where we check localStorage for the existance of a token. If the token does not exist, we will redirect the user to the /logout route.
