...
**Required Secrets for Our CI/CD Pipeline:**

You need to configure three secrets in your repository to enable the CI/CD pipeline to deploy your widget:

• **CDN Deployment Secret** - SSH private key for Akamai CDN deployment. This allows the pipeline to upload your built widget files to the content delivery network.

• **Configuration Service Secret** - Azure App Configuration connection string. This enables the pipeline to update configuration metadata when your widget is deployed.

• **Package Registry Secret** - Authentication token for the internal NPM registry. This allows the pipeline to publish your widget package to the internal package registry.
