...
:::caution
An event via Event Bus is part of your widget's API. Since you cannot predict how many host widgets are consuming and utilizing their Event Bus, we must avoid breaking changes to the event payload. It should be treated just as we would props and other API contracts.
...
A hook wrapper around subscribe that handles the React lifecycle. This hook is used by the platform to subscribe to events. This hook will auto-unsubscribe when the component is unmounted.
